    ...
    <div class="relative min-h-screen w-full flex items-center justify-center px-4 py-12"
        ...
        <div class="relative z-10 w-full max-w-md">
            ...
            <div class="bg-white/5 backdrop-blur-[1px] rounded-[16px] shadow-[0_8px_32px_rgba(0,0,0,0.3)] border border-white/25 p-8 fade-in-up delay-2">
                <h2 class="text-xl font-bold text-white mb-1 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]" style="font-family: 'Quicksand', sans-serif;">Daftar Akun</h2>
                <p class="text-xs text-white/80 mb-6 drop-shadow-[0_1px_3px_rgba(0,0,0,0.5)]">Buat akun untuk mulai memindai sampah dan melacak riwayat deteksi.</p>
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="name" class="block text-xs font-semibold text-white mb-1.5 drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">{{ __('Nama') }}</label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-xs font-semibold text-white mb-1.5 drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">Konfirmasi Kata Sandi</label>
                        <div class="relative">
                    ...
                    @if (Laravel\Jetstream\Jetstream::hasTermsAndPrivacyPolicyFeature())
                        <div class="pt-1">
                            <label for="terms" class="flex items-start gap-2 text-xs text-white/85 cursor-pointer drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
                                <input type="checkbox" name="terms" id="terms" required
                                    class="mt-0.5 rounded border-white/40 bg-white/10 text-[#3F51B5] focus:ring-white">
                                <span>
                                    {!! __('Saya menyetujui :terms_of_service dan :privacy_policy', [
                                            'terms_of_service' => '<a target="_blank" href="'.route('terms.show').'" class="underline font-medium text-white hover:text-[#CFCDFC]">'.__('Syarat Layanan').'</a>',
                                            'privacy_policy' => '<a target="_blank" href="'.route('policy.show').'" class="underline font-medium text-white hover:text-[#CFCDFC]">'.__('Kebijakan Privasi').'</a>',
                                    ]) !!}
                                </span>
                            </label>
                        </div>
